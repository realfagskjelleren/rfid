...
public class MySQLDBHandler implements DBHandler {
    ...
    private final String UPDATE_USER_RFID_QS = "UPDATE user SET rfid = ? WHERE rfid = ?";
    private final String GET_USER_RFID_QS = "SELECT id, rfid, credit, is_staff, ecc, last_used FROM user WHERE rfid = ?;";
    private final String GET_USER_ECC_QS = "SELECT id, rfid, credit, is_staff, ecc, last_used, created FROM user WHERE ecc = ?;";
    ...
    private final String CREDITS_QS = "SELECT credit FROM user WHERE rfid = ?;";
    ...
    private final String GET_TRANSACTIONS_QS = "SELECT rowid, user_id, value, is_deposit, date FROM transact ORDER BY rowid DESC LIMIT ?;";
    private final String GET_ALL_USERS_QS = "SELECT * FROM user;";

    private final String TRANSACTION_QS = "INSERT INTO transact (user_id, value, is_deposit, new_balance) VALUES (?, ?, ?, ?);";
     ...
     * If no user matches teh given rfid, one will be created first.
     *
     ...
     * The function of ECC is to give users a small token which identifies their account, in case of
     * card loss or other needs for replacement.
     *
    ...
    @Override
    public void update_user_rfid(int user_id, String rfid) {
